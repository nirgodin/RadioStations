...
class GenderAnalyzer(IAnalyzer):
    def __init__(self):
        self._prioritized_sources = {
            DataSource.MANUAL_TAGGING: self._read_and_map_df_columns(MANUAL_GENDERS_TAGGING_PATH),
            DataSource.SPOTIFY_EQUAL_PLAYLISTS: self._read_and_map_df_columns(SPOTIFY_ISRAEL_GLOBAL_EQUAL_PLAYLISTS_OUTPUT_PATH),
            DataSource.ISRAELI_WIKIPEDIA: self._read_and_map_df_columns(WIKIPEDIA_ISRAELI_ARTISTS_GENDER_PATH),
            DataSource.OPENAI: self._read_and_map_df_columns(OPENAI_GENDERS_PATH),
            DataSource.GENERAL_WIKIPEDIA: self._read_and_map_df_columns(WIKIPEDIA_OPENAI_UNKNOWN_GENDERS_PATH),
            DataSource.GOOGLE_IMAGES: self._get_google_images_classifications()
        }
